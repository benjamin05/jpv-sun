...
class ImportClasificationArticleDialog extends JDialog {

  private static final String TXT_DIALOG_TITLE = MainWindow.TEXT_LOAD_PART_CLASS_TITLE
  ...
  Component parent
    ...
    ImportClasificationArticleDialog( ) {
  ...
  protected void startImport( ) {
    while( !this.filesToLoad.isEmpty() ) {
      ...
      IOController.getInstance().dispatchImportClasificationArt(f)
  ...
  protected void refreshUI( ) {
    ...
    if ( !this.filesToLoad.isEmpty() ) {
        this.txtFiles.text = filesToLoad.toString()
    }
  }
  ...
  protected void onButtonImport( ) {
    logger.debug( 'ImportClasificationArticles confirmed' )
